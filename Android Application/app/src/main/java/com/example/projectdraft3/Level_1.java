...
public class Level_1 extends AppCompatActivity {
    ...
    DatabaseReference level_one = root.child("LevelOne");
    String names;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        quit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                finish();
                startActivity(new Intent(Level_1.this, LevelSelect.class));
        ...
        final String[] queses = {"I know everything but im unreliable. They warn you to not use me but im your main " +
                "source of information for everything anyway. What am I?",
                "You can buy almost everything from me and have it delivered straight to your home" +
                        ", as a fun fact, i started of as an online book store. What am I?",
                "Cat videos, car videos, music and movie trailers, every video is free to watch and most " +
                        "people use me daily. What am I?"};

        final String[] urls = {"https://www.wikipedia.org/",
        ...
        results.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (notCompleted) {
                    ...
                    finish();
                    startActivity(new Intent(Level_1.this, Menu.class));
                    overridePendingTransition(android.R.anim.fade_in, android.R.anim.fade_out);
                }
            }
        });
                    ...
                    if(putInBase) {
                        if (names.contains("First")) {
                            if (names.contains("none")) {
                                level_one.child("First").child("Name").setValue(input);
                            } else {
                                level_one.child("First").child("Name").setValue(names.split(" ", 2)[1] + input);
                            }
                        }
                        else if (names.contains("Second")) {
                            if (names.contains("none")) {
                                level_one.child("Second").child("Name").setValue(input);
                            } else {
                                level_one.child("Second").child("Name").setValue(names.split(" ", 2)[1] + input);
                            }
                        }
                        else if (names.contains("Third")) {
                            if (names.contains("none")) {
                                level_one.child("Third").child("Name").setValue(input);
                            } else {
                                level_one.child("Third").child("Name").setValue(names.split(" ", 2)[1] + input);
                            }
                        }
                    }
                    ...
                    public void run() {
                        ...
                        if (++seconds % 60 == 0) {
                            ...
                            if (minutes == 5) {
                                ...
                                T.cancel();
                                if (!sharedPrefs.contains("Level One Score")) {
                                    int score = 0;
                                    score += 60 * minutes * 1.5;
                                    score += seconds;
                                    editor.putInt("Level One Score", 500 - score);
                                    level_one.child("Tmp").setValue(500 - score);
                                }
                                editor.apply();
                            }
                        }
        ...
        level_one.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (tmp == third && !putInBase) {
                    ...
                    level_one.child("Tmp").setValue(-1);
                }
                else if (third < tmp && tmp <  second && !putInBase) {
                    putInBase = true;
                    names = dataSnapshot.child("Third").child("Name").getValue(String.class);
                    level_one.child("Third").child("Value").setValue(tmp);
                    ...
                    putInBase = true;
                    names = dataSnapshot.child("Second").child("Name").getValue(String.class);
                    level_one.child("Second").child("Value").setValue(tmp);
                    ...
                    putInBase = true;
                    names = dataSnapshot.child("First").child("Name").getValue(String.class);
                    level_one.child("First").child("Value").setValue(tmp);
